...
$$.init_anon = {"":
 ...
 call$1: function(event$) {
  $.window().requestAnimationFrame$1($.run);
}
};
...
$.update = function(time) {
  if ($.gtB($.sub(time, $.lastUp), $.updateTimer)) {
    $.lastUp = $.DateImplementation$now().millisecondsSinceEpoch;
    $.movePlayer();
  }
};
...
$.checkCollision = function(dx, dy) {
  if ($.gtB($.add($.add($.x, dx), $.imgSizeX), 800) || $.ltB($.add($.x, dx), 0) || $.gtB($.add($.add($.y, dy), $.imgSizeY), 600) || $.ltB($.add($.y, dy), 0))
    return false;
  else
    return true;
};
...
$.run = function(time) {
  if (time == null)
    time = $.DateImplementation$now().get$millisecond();
  $.update(time);
  $.animate(time);
  $.render();
  $.window().requestAnimationFrame$1($.run);
...
$.animate = function(time) {
  if ($.moved === true && $.gtB($.sub(time, $.lastAnim), $.animTimer)) {
    $.stateX = $.add($.stateX, 1);
    $.stateX = $.mod($.stateX, $.animSizeX);
    $.lastAnim = $.DateImplementation$now().millisecondsSinceEpoch;
  }
};
...
$.movePlayer = function() {
  ...
  if ((!(dx === 0) || !(dy === 0)) && $.checkCollision(dx, dy) === true) {
    $.x = $.add($.x, dx);
    $.y = $.add($.y, dy);
    $.moved = true;
  } else
...
$.render = function() {
  $.ctx.drawImage$9($.img, $.mul($.imgSizeX, $.stateX), $.mul($.imgSizeY, $.stateY), $.imgSizeX, $.imgSizeY, $.x, $.y, $.imgSizeX, $.imgSizeY);
};
...
$.init = function() {
  ...
  $.img = $._ElementFactoryProvider_Element$tag('img');
  $.img.set$src('img/player.png');
  $.add$1($.img.get$on().get$load(), new $.init_anon());
  $.add$1($.document().get$body().get$on().get$keyDown(), new $.init_anon0());
  $.add$1($.document().get$body().get$on().get$keyUp(), new $.init_anon1());
  $.updateTimer = 33.333333333333336;
};
